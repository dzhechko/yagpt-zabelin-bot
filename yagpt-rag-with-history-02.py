...
def main():
    ...
    st.set_page_config(page_title="Материаловед чат-бот", page_icon="📖")   
    # Отображаем лого измененного небольшого размера
    st.image(resized_logo)
    st.title('📖 Материаловед чат-бот')
    """
    ...
    if len(msgs.messages) == 0:
        msgs.add_ai_message("Привет, я эксперт по материаловедению! Чем могу вам помочь?")
    ...
    Не вздумай добавлять какие-то пояснения к "Мне не удалось найти ответ в ваших документах". 
